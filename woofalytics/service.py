...
class BarkMonitor:
    def __init__(self, config: AppConfig):
        ...
        self._status = BarkStatus(threshold=config.bark_threshold)
        ...
        if config.demo_mode:
            ...
            self._detector = YamnetTFLiteBarkDetector(
                ...
                target_indices=config.bark_class_indices,
            )
    ...
    def _run(self) -> None:
        ...
        self._logger.info(
            "Starting bark monitor at %s Hz with threshold %.2f",
        ...
        while not self._stop_event.is_set():
            ...
            while len(self._analysis_buffer) >= self._config.inference_window_samples:
                ...
                if inference.is_bark and self._trigger_gate.allow():
    ...
    def _run_demo_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            inference = BarkInference(
                ...
                threshold=self._config.bark_threshold,
                target_scores={
                    "bark": bark_score,
    ...
    def _update_status(self, inference: BarkInference) -> None:
        ...
        with self._status_lock:
            ...
            self._status.threshold = inference.threshold
        ...
        body = json.dumps(
            {
                ...
                "value2": dt.datetime.now(dt.UTC).isoformat(),
                "value3": max(
                    inference.target_scores, key=inference.target_scores.get, default=""
                ),
            }
        ).encode("utf-8")
        url = (
